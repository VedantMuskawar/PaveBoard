 ...
 * This wrapper function handles the defOrderID + deliveryDate duplicate check
 * @param {Object} order - The SCH_ORDERS document data
 * @returns {Promise<{success: boolean, dmNumber?: number, error?: string}>}
 */
export async function generateDeliveryMemoSafe(order) {
  try {
    // Pre-transaction check: Ensure no other SCH_ORDERS doc with same defOrderID + deliveryDate has a DM
    const duplicateCheckQuery = query(
      collection(db, "SCH_ORDERS"),
      where("defOrderID", "==", order.defOrderID),
      where("deliveryDate", "==", order.deliveryDate),
    ...
    if (conflictingDocs.length > 0) {
      ...
      throw new Error(
        `Duplicate prevention: Another order (${conflictingDoc.id}) with the same defOrderID (${order.defOrderID}) and delivery date already has DM #${conflictData.dmNumber}`
      );
    }
